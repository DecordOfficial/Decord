...
async function patchGuildMember(guildId: string, userId: string, body: any, successMsg: string) {
    ...
    if (!token) {
        Toasts.show({ message: "Could not get auth token", type: Toasts.Type.FAILURE, id: Toasts.genId() });
        return;
    }
    ...
    try {
        ...
        if (response.ok)
            ...
            Toasts.show({ message: `Operation failed (${response.status})`, type: Toasts.Type.FAILURE, id: Toasts.genId() });
    } catch {
        Toasts.show({ message: "A network error occurred", type: Toasts.Type.FAILURE, id: Toasts.genId() });
    }
}
...
export default definePlugin({
    ...
    description: "Automatically mutes, deafens, or disconnects the user.",
        ...
        VOICE_STATE_UPDATES({ voiceStates }: { voiceStates: VoiceState[] }) {
            for (const state of voiceStates) {
                ...
                if (settings.store.disconnectUserId === userId && channelId)
                    void patchGuildMember(guildId, userId, { channel_id: null }, "User disconnected");

                // Mute check
                if (settings.store.muteUserId === userId && mute === false)
                    void patchGuildMember(guildId, userId, { mute: true }, "User muted");

                // Deafen check
                if (settings.store.deafenUserId === userId && deaf === false)
                    void patchGuildMember(guildId, userId, { deaf: true }, "User deafened");
            }
        }
    }
});
